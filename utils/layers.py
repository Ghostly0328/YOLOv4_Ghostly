...
class RegP(nn.Module): # 應用層 http://dx.doi.org/10.14311/nnw.2019.29.004
    def __init__(self, kernel_size = 3, stride = 1, padding = 0):
        super(RegP, self).__init__()
...
class RegMean(nn.Module): # 應用層 http://dx.doi.org/10.14311/nnw.2019.29.004
    def __init__(self, kernel_size = 3, stride = 1, padding = 0, isclose = 30e-02, reflect = False):
        super(RegMean, self).__init__()
        self.k = _pair(kernel_size)
        self.stride = _pair(stride)
        self.padding = _quadruple(padding)
