...
#         TFcounter = torch.isclose(arrayA, arrayB, rtol=275e-03, atol= 0).cuda() #isclose 範圍調整    #https://runebook.dev/zh-CN/docs/pytorch/generated/torch.isclose
#         NumberCounter = torch.sum(TFcounter, (4,5)).cuda().type(torch.half) - 1

#         return NumberCounter

#Back
# class RegpPooling2DOld(nn.Module): # 應用層 http://dx.doi.org/10.14311/nnw.2019.29.004
#     def __init__(self, kernel_size = 3, stride = 1, padding = 0):
#         super(RegpPooling2DOld, self).__init__()
#         self.k = _pair(kernel_size)
#         self.stride = _pair(stride)
#         self.padding = _quadruple(padding)
...
#         #Calculate
#         gt = torch.gt(NumberCounter, maxCounter)
#         eq = torch.eq(NumberCounter, maxCounter)

#         p = torch.where(eq, (_CompareSamllX[:,:,:,:,1,1]+ p)/2, p)
#         p = torch.where(gt, _CompareSamllX[:,:,:,:,1,1], p)
#         maxCounter = torch.where(gt, NumberCounter, maxCounter)

#         return p,maxCounter

#     def _padding6D(self, array): #進行6D維度擴展 複製成相同數字
#         array = array[:,:,:,:,1,1].unsqueeze(-1).unsqueeze(-1)
#         #array = torch.cat((array, array,array), 4) #複製
#         #array = torch.cat((array, array,array), 5) #複製
#         return array.repeat(1,1,1,1,3,3)

#     def CompareTF(self, arrayA, arrayB): #比較
        
#         TFcounter = torch.isclose(arrayA, arrayB, rtol=20e-02, atol= 0) #isclose 範圍調整    #https://runebook.dev/zh-CN/docs/pytorch/generated/torch.isclose
#         NumberCounter = torch.sum(TFcounter, (4,5), dtype=torch.half) - 1
...
class RegpPooling2D(nn.Module): # 應用層 http://dx.doi.org/10.14311/nnw.2019.29.004
    ...
    def _RegpPoooling(self, x):
        ...
        array = torch.isclose(x, array, rtol=20e-02, atol= 0) #isclose 範圍調整    #https://runebook.dev/zh-CN/docs/pytorch/generated/torch.isclose
